...
type GrokOAuthCapabilities struct {
	...
	MaxImportConcurrency int  `json:"max_import_concurrency"`
}

func (s *GrokOAuthService) GetCapabilities() GrokOAuthCapabilities {
	return GrokOAuthCapabilities{
		PasswordAuthEnabled:  s.passwordAuthEnabled(),
		CaptchaConfigured:    grokCaptchaConfigured(),
		MaxImportConcurrency: 10,
	}
}

func grokCaptchaConfigured() bool {
	return strings.TrimSpace(os.Getenv("YESCAPTCHA_CLIENT_KEY")) != "" ||
		strings.TrimSpace(os.Getenv("YESCAPTCHA_API_KEY")) != ""
}
