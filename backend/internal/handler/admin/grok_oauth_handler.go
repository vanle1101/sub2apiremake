...
const (
	grokSSOImportDefaultConcurrency = 3
	grokSSOImportMaxConcurrency     = 10
)
...
func (h *GrokOAuthHandler) CreateAccountsFromSSO(c *gin.Context) {
	...
	if hasGrokPasswordImport(tokens) {
		capabilities := h.grokOAuthService.GetCapabilities()
		if !capabilities.PasswordAuthEnabled {
			response.BadRequest(c, "Grok password authorization is disabled")
			return
		}
		if !capabilities.CaptchaConfigured {
			response.BadRequest(c, "GROK_OAUTH_CAPTCHA_KEY_REQUIRED: configure YESCAPTCHA_CLIENT_KEY on the server before importing email/password accounts")
			return
		}
	}

	ctx := c.Request.Context()
	workerCount := normalizeGrokImportConcurrency(req.ImportConcurrency)
...
func (h *GrokOAuthHandler) createAccountFromSSOToken(ctx context.Context, req GrokSSOToOAuthRequest, token string, index, total int) grokSSOImportWorkerResult {
	...
	if email, password, ok := parseGrokPasswordImport(token); ok {
		tokenInfo, err = h.grokOAuthService.AuthorizePassword(ctx, email, password, req.ProxyID)
	} else {
...
func normalizeSSOImportTokens(tokens []string, single string) []string {
	...
	for _, item := range items {
		// Passwords may contain commas, semicolons, pipes, or dashes. Split only on
		// physical lines and detect email/password before applying SSO normalization.
		parts := strings.Split(strings.ReplaceAll(item, "\r", "\n"), "\n")
...
func parseGrokPasswordImport(value string) (email, password string, ok bool) {
	...
	email = strings.TrimSpace(value[:index])
	password = value[index+len(separator):]
	if !strings.Contains(email, "@") || password == "" || strings.HasPrefix(strings.ToLower(email), "sso-") {
		return "", "", false
	}
	return email, password, true
}

func hasGrokPasswordImport(tokens []string) bool {
	for _, token := range tokens {
		if _, _, ok := parseGrokPasswordImport(token); ok {
			return true
		}
	}
	return false
...
func normalizeGrokImportConcurrency(requested int) int {
	if requested <= 0 {
		return grokSSOImportDefaultConcurrency
	}
	if requested > grokSSOImportMaxConcurrency {
		return grokSSOImportMaxConcurrency
	}
	return requested
}
