...
func TestBuildAuthorizationURL_MissingRuntimeSecret(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("BuildAuthorizationURL() should reject a missing runtime client secret; URL: %s", authURL)
	}
}
...
func TestEffectiveOAuthConfig_NoEnvSecret(t *testing.T) {
	...
	if err == nil {
		t.Fatal("missing runtime client secret should return an error")
	}
}
