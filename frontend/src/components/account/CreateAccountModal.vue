...
import type { GrokSSOToOAuthResponse } from '@/api/admin/grok'
...
const GROK_IMPORT_CHUNK_SIZE = 30
const GROK_IMPORT_CONCURRENCY = 5

const handleGrokImportSSO = async (ssoInput: string) => {
  // Preserve password bytes after the separator. Deduping here also keeps result
  // indexes aligned with the source lines returned to the textarea on failure.
  ...
  grokOAuth.loading.value = true
  grokOAuth.error.value = ''
  grokImportProgress.value = `Preparing ${ssoTokens.length} account(s)...`
  ...
  try {
    for (let offset = 0; offset < ssoTokens.length; offset += GROK_IMPORT_CHUNK_SIZE) {
      const chunk = ssoTokens.slice(offset, offset + GROK_IMPORT_CHUNK_SIZE)
      grokImportProgress.value = `Importing ${offset + 1}-${offset + chunk.length} of ${ssoTokens.length}...`

      let result: GrokSSOToOAuthResponse
      try {
        result = await adminAPI.grok.createFromSSO({
          sso_tokens: chunk,
          ...
          expires_at: form.expires_at,
          auto_pause_on_expired: autoPauseOnExpired.value,
          import_concurrency: GROK_IMPORT_CONCURRENCY
        })
      } catch (error: any) {
        failedLines.push(...ssoTokens.slice(offset))
        errors.push(error.response?.data?.detail || error.message || t('admin.accounts.oauth.grok.failedToConvertSSO'))
        break
      }

      successCount += result.created?.length || 0
      for (const item of result.failed || []) {
        const sourceLine = chunk[item.index - 1]
        if (sourceLine !== undefined) failedLines.push(sourceLine)
        errors.push(`#${offset + item.index}: ${item.error || 'Unknown error'}`)
      }
    }

    const failedCount = failedLines.length
    if (oauthFlowRef.value) oauthFlowRef.value.ssoCookie = failedLines.join('\n')
    if (successCount > 0 && failedCount === 0) {
      appStore.showSuccess(
        ssoTokens.length > 1
          ? t('admin.accounts.oauth.batchSuccess', { count: successCount })
