        ...
        <div v-if="inputMethod === 'sso_cookie'" class="space-y-4">
          ...
          >
            <p class="mb-3 text-sm text-blue-700 dark:text-blue-300">
              {{
                platform === 'grok'
                  ? 'Paste all accounts at once, one per line: email|password, email----password, or a Grok Web SSO key.'
            ...
            <div class="mb-4">
              ...
              >
                <Icon name="key" size="sm" class="text-blue-500" />
                {{ platform === 'grok' ? 'Grok SSO / Email + Password' : t(getOAuthKey('ssoCookieLabel')) }}
                <span
              ...
              <p class="mt-1 text-xs text-blue-600 dark:text-blue-400">
                {{
                  platform === 'grok'
                    ? 'One account per line. A large list is automatically split into safe batches; failed lines remain for retry.'
                    : t(getOAuthKey('ssoCookieHint'))
                }}
              </p>
              <p
                v-if="platform === 'grok' && !captchaConfigured"
                class="mt-2 text-xs font-medium text-amber-600 dark:text-amber-400"
              >
                Email/password login requires YESCAPTCHA_CLIENT_KEY on the server. Raw SSO keys do not.
              </p>
            </div>
            ...
            >
              <p class="text-sm text-blue-700 dark:text-blue-300">{{ progress }}</p>
            </div>
...
const captchaConfigured = ref(false)
...
watch(
  () => [props.platform, props.showEmailPasswordOption, props.showSsoOption] as const,
  async ([platform, requested, ssoRequested]) => {
    passwordAuthEnabled.value = false
    captchaConfigured.value = false
    if (platform !== 'grok' || (!requested && !ssoRequested)) return
    try {
      const capabilities = await adminAPI.grok.getCapabilities()
      passwordAuthEnabled.value = capabilities.password_auth_enabled
      captchaConfigured.value = capabilities.captcha_configured
    } catch {
...
const handleImportSSO = () => {
  if (ssoCookieInput.value.trim()) {
    emit('import-sso', ssoCookieInput.value)
  }
}
