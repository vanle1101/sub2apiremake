...
export function getGrokSSOImportTimeout(keyCount: number, concurrency = GROK_SSO_IMPORT_CONCURRENCY): number {
  const batches = Math.ceil(Math.max(1, keyCount) / Math.max(1, concurrency))
...
export async function createFromSSO(payload: GrokSSOToOAuthRequest): Promise<GrokSSOToOAuthResponse> {
  const { data } = await apiClient.post<GrokSSOToOAuthResponse>(
    ...
    payload,
    { timeout: getGrokSSOImportTimeout(payload.sso_tokens.length, payload.import_concurrency) }
  )
  return data
}
